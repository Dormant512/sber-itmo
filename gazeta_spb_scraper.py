...
def get_news_from_date(cur_date, paginator=1):
...
if __name__ == "__main__":
    ...
    with open('sber_work_files/gazeta_spb_news.csv', 'w', newline='') as csvfile:
        ...
        for single_date in daterange(start_date, end_date):
            ...
            for page in range(1, 31):
                try:
                    page_news = get_news_from_date(single_date, page)
                    cur_news.extend(page_news)
                    print(f"News for {single_date}, page {page} scraped.")
                except:
                    print(f"Page {page} does not exist. Finishing day {single_date}.\n")
                    break
